<template>
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-between flex-col gap-4 text-black p-10 bg-slate-400 rounded-lg z-10" >
        ...
        <div class="flex gap-4">
            ...
            <UInput type="email" name="email" placeholder="Enter Your Email" class=" h-7 pl-2" v-model="email" />
        </div>
...
const siginHandler = async() => {
    const user = await userStore.signIn(email.value, password.value)
    email.value = ""
    password.value = ""
    if (user.status === 200) {
        router.push('/dashboard')
